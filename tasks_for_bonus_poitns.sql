...
BEGIN
    ...
    UPDATE accounts
    SET balance = balance - (SELECT COALESCE(SUM((rec.amount)::DECIMAL),0)
                             FROM jsonb_to_recordset(p_payments) AS rec(iin VARCHAR, amount DECIMAL, description TEXT)
                             WHERE rec.iin NOT IN (SELECT (elem->>'iin') FROM jsonb_array_elements(v_failed_details) elem))
...
-- Scenario A: Successful Transfer (John Doe -> Jane Smith)
-- Expected: Success, balances updated, transaction logged
DO $$
    BEGIN
        RAISE NOTICE 'Testing Successful Transfer...';
        CALL process_transfer('KZ001KZT', 'KZ002EUR', 10000, 'KZT', 'sadaka');
    END $$;

-- Scenario B: Failed Transfer (Insufficient Funds)
-- Expected: Error 'Insufficient funds', entry in audit_log
DO $$
    BEGIN
        RAISE NOTICE 'Testing Insufficient Funds...';
        CALL process_transfer('KZ001KZT', 'KZ002EUR', 99999999, 'KZT', 'Impossible transfer');
    EXCEPTION WHEN OTHERS THEN
        RAISE NOTICE 'Caught expected error: %', SQLERRM;
    END $$;

-- Scenario C: Failed Transfer (Daily Limit Exceeded)
-- Pre-requisite: Set a low limit for testing or make a huge transfer
UPDATE customers SET daily_limit_kzt = 1000 WHERE iin = '900101400500'; -- John Doe
DO $$
    BEGIN
        RAISE NOTICE 'Testing Daily Limit...';
        CALL process_transfer('KZ001KZT', 'KZ002EUR', 5000, 'KZT', 'Limit breaker');
    EXCEPTION WHEN OTHERS THEN
        RAISE NOTICE 'Caught expected error: %', SQLERRM;
    END $$;
-- Restore limit
UPDATE customers SET daily_limit_kzt = 200000 WHERE iin = '900101400500';

-- Scenario D: Batch Salary Processing
-- Expected: Success, returns JSON summary
DO $$
    DECLARE
        v_result JSONB;
        v_batch JSONB := '[
        {"iin": "900101400500", "amount": 50000, "description": "Salary John"},
        {"iin": "999999999999", "amount": 50000, "description": "Unknown User"}
    ]';
    BEGIN
        RAISE NOTICE 'Testing Batch Salary...';
        CALL process_salary_batch('KZCOMP001', v_batch, v_result);
        RAISE NOTICE 'Batch Result: %', v_result;
    END $$;


-- 1. Для Covering Index (поиск транзакций клиента)
EXPLAIN ANALYZE
SELECT amount, status
FROM transactions
WHERE from_account_id = 1 AND created_at > '2023-01-01';

-- 2. Для Partial Index (поиск только активных счетов)
EXPLAIN ANALYZE
SELECT balance
FROM accounts
WHERE account_number = 'KZ001KZT' AND is_active = TRUE;

-- 3. Для Expression Index (поиск по email без учета регистра)
EXPLAIN ANALYZE
SELECT * FROM customers
WHERE LOWER(email) = 'ali@gmail.com';

-- 4. Для GIN Index (поиск по JSON логам)
EXPLAIN ANALYZE
SELECT * FROM audit_log
WHERE new_values @> '{"amount": 12345}';
